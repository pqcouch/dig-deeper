---
name: dig-deeper
description: "Run the Dig Deeper 16-tool exegetical toolkit on a Bible passage, plus seven extensions (original languages, textual variants, historical background, biblical theology, difficult verses, Christological reading, original audience reception). Always trigger when the user says dig deeper, run the toolkit, analyse this passage, prepare to preach, or wants to understand a passage before preaching. Also trigger for claim audit tasks - checking whether allusions hold, testing intertextual claims, auditing a book overview, or testing the claims of a new commentary / article / lecture before incorporating them into an overview or sermon. Also trigger for macro-synthesis / cross-pericope synthesis — drawing together the wider arguments a completed set of dig-deeper runs jointly demonstrates (e.g. \"what do all these runs add up to\", \"pull the threads together\"). Do not attempt serious passage-level exegetical work without this skill. Complements book-overview and point-purpose. Default translation: ESV."
---
...
| Required | Notes |
|----------|-------|
| **Passage reference** | E.g. "Ruth 1:1–22", "John 3:16", "Romans 8:1–11" |
...
**Choosing the mode (Fresh Exegesis vs Claim Audit vs Macro-Synthesis):**
...
**Depth floors (solo Fresh Exegesis runs).** Adaptive depth is a ceiling-setter, not a licence to thin the report. For a solo run, the following are minimums, whatever model is executing:
...
**1 — Translation loss (not a variant at all).** The Hebrew carries a literary feature no translation into any language can carry: a repeated root, a wordplay, a Leitwort, a count of formulaic occurrences, a pun on a name. The Greek "breaks" it because *translation* breaks it — the ESV breaks it too.
...
- **Stays text-first** — every finding traceable to the passage, not deduced from elsewhere
