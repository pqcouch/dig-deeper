...
### Worked examples — in the prep folder, not in this skill

**The current calibration anchors live in `_skill-examples/` in the connected Dig Deeper prep folder**, beside `_texts/`. Read them from there.

| File | Purpose |
|------|---------|
| `_skill-examples/psalm-33-worked.md` | Full current-standard report on an OT poetry passage (Psalm 33:1–9). Demonstrates the Positional Necessity Check on a psalm, the OT Citation Triad with a Psalter-level Move 2, warrant tagging, the Text-First Declaration, and **the script-and-gloss convention in Hebrew throughout**. **Primary example for OT poetry.** |
| `_skill-examples/romans-8-31to39-worked.md` | Full current-standard report on an NT epistle passage with OT citations (Romans 8:31–39). Demonstrates Phase 0.6, the OT Citation Triad (Moves 1–3), warrant tagging, the complete report format, and **the convention in Greek throughout**. **Primary worked example.** |

> **The copies bundled at `examples/` inside this skill are superseded and must not be used as the anchor.** They predate Round 4 and contain no Hebrew at all — a run that matched them would reproduce transliteration in place of the script, and would inherit a stated count that the corpus has since shown to be wrong. If `_skill-examples/` is unreachable because the prep folder is not connected, say so, and treat the bundled copies as a guide to *structure and depth only*, never to original-language style or to any count they state.
...
| 1 | **View the seven core tool files** in `references/`. View `historical-background.md`, `original-audience.md`, and `preacher-extras.md`. View the genre-matching worked example from **`_skill-examples/` in the prep folder** as the calibration anchor (see Required reading). |
...
**Adopted September 2026; trial completed 11 September 2026.** The mode was adopted on one run and tested on a second — the Isa 36–39 ‖ 2 Kgs 18–20 pair, the *open* case, which is the one that can break it. It held: the diff established the extent of identity, the direction was declared and not resolved, and Phase 0.6 run twice produced two distinct positional answers. **The evidence base is two runs, both on the same library**, so treat the mode as working method rather than as settled beyond revision — and the withdrawal test still stands: if a synoptic report turns out worse than two separate digs would have been, say so and withdraw it.
...
**2. Set the extent before working, at each book's own seams — not at the overlap.** Align the two references first; they are frequently off by a verse or a chapter, and a supplied pair should be checked rather than trusted (2 Kgs 18:17 corresponds to Isa 36:2, not to Isa 37). **Then set each boundary at the natural seam in its own book, not at the point where the texts begin and cease to run together.** The purpose is to bring material unique to one side *inside* the run: on an open direction of dependence an absence is evidence, and a boundary drawn at the overlap systematically excludes exactly the evidence the mode exists to weigh — 2 Kgs 18:14–16, the tribute and the stripping of the temple doors, has no parallel in Isaiah at all and carries a headline finding. Record the extent decision and its reason in the scope note. Where the extent is genuinely contestable, say which alternative was rejected and what it would have excluded.

**3. Work in three movements.** Passage A in its own frame; passage B in its own frame; then the comparison. Do not average the two into a composite: **the harmonised text exists nowhere and is the mode's first pitfall.** Where one passage already has a completed solo dig, consume it at Phase 0.5 rather than re-working it, and say so in the scope note.

**4. Diff mechanically before reading either text closely, and diff in two passes, not one.** Align the passages verse by verse. **Pass 1 is *consonantal*:** NFD-strip combining marks, normalise maqqef ־, paseq ׀ and sof pasuq ׃ to spaces (gate item (e3)), reduce to consonants and spaces. **Pass 2 is *skeletal*:** additionally delete waw, yod and word-final *he*, so that plene/defective spelling collapses to identity. **A pair identical in pass 2 but not in pass 1 differs only in spelling.** Report both figures and stratify what remains by magnitude — identical / orthographic / 1–2 words / 3–6 / 7+ — because **the share of material that is identical *but for spelling* is a finding in its own right**, and because divergence is usually concentrated rather than spread. A one-pass figure is not wrong but it is misleading, and it misleads in the direction that matters: it inflates difference, which is exactly what a synoptic run must not do while the direction of dependence is open. A mechanical diff is reproducible, catches what the eye slides over, and — crucially — **establishes what is identical**, which is as much a finding as what differs. Report the count of unchanged material explicitly.

**5. Run the per-book checks twice — Phase 0.6, Move 4, and house vocabulary.** Each is a fact about one book's own composition; none can be inherited from a twin, and a single pass over "the passage" produces a result belonging to no book.

- **Phase 0.6, twice.** Each passage's positional necessity is a fact about its own book.
- **Move 4, once per book — and the asymmetry between the two results is itself a finding.** An internal echo cannot be shared. Run it twice, report the two lists separately, and **say what each book does with the shared material that the other does not** — including where one book plants something it never resolves and the other resolves something it never planted. Where the two books use the same concrete detail for different ends (a place name serving an echo in one book and an inclusio in the other), that divergence of *use* is reportable even though the *wording* is identical.
- **House vocabulary, in both directions, before concluding the comparison.** Take each book's characteristic vocabulary — the terms a whole-book sweep or overview has identified as its own — and ask three questions of the shared text: (a) does the term appear in one version and not the other, and in which direction; (b) how does it behave across each whole book; (c) is it *rare* in the host book, in which case its presence here is design, or *common*, in which case it is idiom? **Report the direction of every divergence, including the ones that cut against the run's drift.** The test is cheap, mechanical, and is the most direct evidence a synoptic run can obtain about how each book owns the material. In the Hezekiah pair it produced the central finding in both directions at once — יְהוָה צְבָאוֹת three times out of three in Isaiah's version where Kings has plain יְהוָה; נָגִיד, Kings' own royal-designation term, in Kings' oracle where Isaiah has nothing; and קְדוֹשׁ יִשְׂרָאֵל in **both**, at Isa 37:23 = 2 Kgs 19:22, its only occurrence in the whole of Joshua–Kings. Two data pointing one way, one the other — and that symmetry is what turned "open" from a refusal into a result.

**6. Expect the Triad to behave differently.** Where the source is inside the same corpus as the citing text (Deuteronomy quoting Exodus), Move 2 (book usage) and Move 4 (internal echo) largely coincide. Say so rather than filing a nil return for one of them.

**7. Reconcile against *both* book-overviews, and report the asymmetry.** Two overviews are live and they are rarely equally mature. Where one is materially behind the other — older, pre-corpus, built on a sweep rather than on solo digs — **that gap is a finding for the Book-Overview Tensions section**, because the weaker overview will systematically lack whatever the run's original-language work produced.

**8. File it where both books can find it.** A report spanning two books goes at the level above both **where the two share a parent** — the `Torah/` root for Exod // Deut, the corpus root for a pair spanning sections. **Where they share no parent** (Isa // Kgs, 2 Sam // Ps, Obad // Jer, Sam–Kgs // Chr — which is most of the known pairs), the real file goes in **one** book's folder and a **one-line pointer page** goes in the other's, naming the report and its path. Choose the host folder by where the *unique* material sits: Hezekiah's psalm (Isa 38:9–20) has no counterpart in Kings, so the pair is hosted by `Isaiah/`. **Do not keep two copies** — they drift, and the library index lists the report twice. Filename per the two-passage convention below, either way.
...
4. **Match the worked example's standard, not your instinct.** The genre-matching example in **`_skill-examples/` in the prep folder** is the calibration anchor for depth, register, flagging density and original-language style. Deviations should be passage-driven, not model-driven. **The superseded copies at `examples/` are not the anchor** — see Reference Materials.
...
2. **Flag confidence levels explicitly:**
   - `*High confidence:*` for well-known data (e.g., חֶסֶד ("lovingkindness") and its range; "I AM" in John 8 alluding to Exodus 3)
...
3. **When uncertain, ask the user or suggest web search.** Frame it as a choice when the claim materially affects the report:
   > "I'm not certain whether the Greek behind 'redemption' here, ἀπολύτρωσις ('redemption, release'), is the same word used in Ephesians 1:7. Want me to (a) attempt + flag, (b) get you to check a lexicon, or (c) web-search to confirm?"
...
**Give the original word in its own script, with an English gloss in brackets immediately after — every occurrence, not only the first.** שָׁכַן ("to dwell"), חֶסֶד ("lovingkindness"), λόγος ("word"), φρονέω ("to think, to be minded").

The gloss is the **NASB95** rendering where the word is being discussed as it stands in a particular verse, and a lexical gloss where it is being discussed as a word. Give the form that actually stands in the text where the form is the point — חַסְדּוֹ ("his lovingkindness"), πέπεισμαι ("I am convinced") — and the lemma where the lexeme is the point. **A chain is checked by lemma, never by the surface form**, and the two are not interchangeable in a report that is making a claim about a chain.

**Transliteration is kept only where the sound is the point** — paronomasia, assonance, alliteration, a name-pun — and then give all three: נְחַשׁ נְחֹשֶׁת ("a serpent of bronze", *nĕḥaš nĕḥōšet*, Num 21:9). A report that transliterates by default makes the reader's Hebrew and Greek harder to learn, not easier; a report that transliterates where the *sound* carries the argument is doing the reader a service. `psalm-33-worked.md` models both — it keeps a transliteration at v.9 for the alliteration and uses the script everywhere else.

**Downstream, the order inverts once.** A sermon manuscript is read aloud, so there the English leads and the original follows in brackets. A dig-deeper report is studied, not spoken; it takes the order above.

The `.odt` and `.html` converters render both scripts and set Hebrew three points above the surrounding English, so nothing special is needed in the Markdown.
...
- *Examples:* תֵּבָה ("basket, ark", Exod 2:3), shared in the Hebrew Bible only with Noah's ark, rendered θῖβις here but κιβωτός at Gen 6:14; פָּנִים ("face, presence") resolved to αὐτός at Exod 33:14, breaking the face/presence hinge.
...
- *Examples:* כַּפֹּרֶת ("mercy seat") → ἱλαστήριον → Rom 3:25; תַּבְנִית ("pattern") → τύπος → Heb 8:5; קָרַן ("to send out rays", Exod 34:29) → δεδόξασται ("has been glorified"), which is what Paul's whole δόξα ("glory") argument in 2 Cor 3:7–18 is reading; Hab 2:4 as quoted at Rom 1:17.
...
- **Hebrew** is set three points above the surrounding English, in the same face.

**Conversion path:** build the Markdown, then render with the canonical house converters in the connected prep folder — `_house-style/make_odt.py` and `_house-style/make_html.py`. They apply the style, declare the house font as an ODF font face (without which LibreOffice silently renders Hebrew in its default complex-script font), set the complex-script size, normalise bare `---` rules so pandoc cannot swallow a section as YAML metadata, and refuse to write a file that has lost a section. If the prep folder is not connected, say so, then generate the `.odt` directly (LibreOffice `soffice --headless`, or a scripted build) and apply every setting above by hand — including the font-face declaration and the complex-script size, which are the two that fail silently rather than visibly. **Before presenting, verify** the delivered file opens in LibreOffice as A5 with Liberation Serif, and that any Hebrew is in that face rather than a fallback.
...
**Scope note (Synoptic Mode only):** [The three movements; **the extent of each passage, where its boundaries were set and why** — naming any alternative extent rejected and what it would have excluded; which passage's frame is supplied by an existing dig rather than re-worked; and **the direction of dependence — stated, open, or contested — with its evidence**.]
...
[Big Bible words, familiar-but-redefined, significant names. Include the proper-noun inventory (one line for routine names) and any name-hyperlink findings — canonical history, outstanding words, name-wordplay — cross-reporting load-bearing ones under Tool 11. Confidence flagged. Originals in their own script with the English gloss in brackets.]
...
[Repeated words, phrases, ideas. Every recurrence claim verified by lemma; counts name their edition.]
...
Then, for the passage as a whole (required even with zero external citations), record the Move 4 internal echo check: `Internal:` findings with direction (*answers §earlier* / *planted for later*) and confidence, or an explicit one-line nil-return ("Internal: none found; opening material checked"). **In Synoptic Mode this is run once per book and reported as two separate lists**, with a line on what the asymmetry between them shows.
...
[Key Hebrew/Greek terms in their own script with English glosses; grammatical/syntactical features; wordplay, with transliteration where the sound is the point; recommendations for verification. Confidence flagged.]
...
[Anything flagged as uncertain — especially anything where lexicon/commentary verification would strengthen the work. Where the corpus has *closed* a question that an earlier pass left open, record it as closed with what was found, rather than deleting it — the record shows the gate working.]
...
**Two-passage (Synoptic Mode):** `dig-deeper-[topic]-[bookA]-[ch]-[bookB]-[ch].odt` — e.g. `dig-deeper-decalogue-exodus-20-deuteronomy-5.odt`, `dig-deeper-hezekiah-isaiah-36-2kings-18.odt`. Named by what the pair is about, because it belongs to neither book alone — and the name stays the same wherever the report is hosted (point 8 of Phase 0.5d).
...
- **Shows the original, not a transliteration of it** — Hebrew and Greek stand in their own script with an English gloss in brackets, so the preacher reads the words themselves; transliteration appears only where the sound carries the argument
...
- **Originals in their own script, with the English gloss in brackets** — every occurrence; transliteration only where the sound is the point. The manuscript stage inverts this order; a report does not.
